...
use crate::{Result, model::WasmMapper};
...
pub trait MapperRepository: Send + Sync {
    ...
    ///
    /// The mapper's `sha256_hash` is used during pipeline execution to verify
    /// `wasm_bytes` integrity.
    ///
    /// # Returns
    /// The created mapper, or an error if creation fails.
