...
pub async fn report_guilds_once(
    ...
    match zako3_tl_client::TlClient::connect(tl_tarpc_addr).await {
        Ok(client) => {
        ...
        Err(e) => {
            tracing::warn!("guild_reporter: failed to connect to TL at {tl_tarpc_addr}: {e}");
        }
    }
}
...
pub async fn run_guild_reporter(
    ctx: serenity::all::Context,
    tl_tarpc_addr: String,
    token: String,
) {
    loop {
        report_guilds_once(&ctx, &tl_tarpc_addr, &token).await;
